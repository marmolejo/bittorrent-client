...
var Client = require('../')
var debug = require('debug')('bittorrent-client:torrent')
...
function Torrent (torrentId, opts) {
  ...
  if (typeof torrentId === 'string') {
    ...
    if (!info.infoHash) {
      return process.nextTick(function () {
        self.emit('error', new Error('invalid torrent uri'))
      })
    }
    ...
    if (torrentId.length === 20) {
      ...
      // torrent file
      metadata = torrentId
      self.infoHash = Client.toInfoHash(torrentId)
    }
  ...
  if (self.client.ready) self._init(metadata)
  else self.client.on('ready', self._init.bind(self, metadata))
}
...
Torrent.prototype._init = function (metadata) {
  var self = this
  debug('init')
  ...
  self.swarm = new Swarm(self.infoHash, self.client.peerId, {
    handshake: {
      dht: !!(self.client.dht && self.client.dhtPort)
    }
  })

  self.swarm.listen(self.client.torrentPort, function (port) {
    self.client.torrentPort = port
    self.emit('listening', port)
    if (metadata) self._onMetadata(metadata)
  })
  ...
  self.swarm.on('download', function (downloaded) {
    self.client.downloadSpeed(downloaded)
  })
  self.swarm.on('upload', function (uploaded) {
    self.client.uploadSpeed(uploaded)
  })
}
...
Torrent.prototype.addPeer = function (addr) {
  ...
  if (self.client.blocklist &&
      (blockedReason = isPeerBlocked(addr, self.client.blocklist))) {
...
Torrent.prototype._onWire = function (wire) {
  ...
  wire.ut_pex.on('dropped', function (peer) {
    ...
    if (!(peer in self.swarm._peers)) self.swarm.remove(peer)
  })
  ...
  if (wire.peerExtensions.dht && self.client.dhtPort) {
    wire.port(self.client.dhtPort)
  }
...
Torrent.prototype._onMetadata = function (metadata) {
  ...
  self.storage.on('file', function (file) {
    debug('storage got complete file: ' + file.name)
  ...
  self.swarm.wires.forEach(function (wire) {
    ...
    wire.ut_metadata.setMetadata(self.metadata)
...
Torrent.prototype._onStorage = function () {
  ...
  if (self.client.trackers && self.parsedTorrent.announce.length > 0) {
    self.tracker = new Tracker(
      self.client.peerId,
      self.client.torrentPort,
      self.parsedTorrent
    )
    ...
    self.tracker.on('peer', function (addr) {
      debug('got peer from tracker: ' + addr)
      self.addPeer(addr)
    })
...
function isPeerBlocked (addr, blocklist) {
  ...
  for (var i = 0, len = blocklist.length; i < len; i++) {
    ...
    if (searchAddr >= startAddress && searchAddr <= endAddress) {
      debug('blocked peer %s because %s', addr, block.reason)
