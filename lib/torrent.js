...
var ut_metadata = require('ut_metadata') // empty object in browser
var ut_pex = require('ut_pex') // empty object in browser
...
Torrent.prototype.addPeer = function (peer) {
  ...
  if (typeof peer === 'string' && self.client.blocked.contains(peer.split(':')[0])) {
...
Torrent.prototype._onWire = function (wire) {
  ...
  if (typeof ut_metadata === 'function') wire.use(ut_metadata(self.metadata))

  if (!self.metadata) {
    if (wire.ut_metadata) wire.ut_metadata.on('metadata', function (metadata) {
      debug('got metadata via ut_metadata')
      self._onMetadata(metadata)
    })
    if (wire.ut_metadata) wire.ut_metadata.fetch()
  }
