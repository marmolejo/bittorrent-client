...
var PeerDiscovery = require('./peer-discovery')
...
function Torrent (torrentId, opts) {
  ...
  if (typeof torrentId === 'string') {
    ...
    if (info.announce) {
      self._announce = typeof info.announce === 'string'
        ? [ info.announce ]
        : info.announce
    }
  ...
  self.swarm.listen(self.client.torrentPort, function (port) {
    ...
    self.discovery = new PeerDiscovery({
    ...
    reemit(self.discovery, self, ['warning', 'dhtAnnounce'])

    self.discovery.on('peer', function (addr) {
      self.addPeer(addr)
    })
